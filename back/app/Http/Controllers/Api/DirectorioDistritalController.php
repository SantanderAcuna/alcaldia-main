...
class DirectorioDistritalController extends Controller
{
    ...
    {
        try {
            $directorio = DirectorioDistrital::with(['categoria', 'tipoEntidad', 'foto'])
                ->filter($request->only(['nombre', 'email']))
                ->orderBy('nombre')
                ->paginate($request->input('per_page', 15))
                ->withQueryString();

            return response()->json(['data' => $directorio], Response::HTTP_OK);
        } catch (\Exception $e) {
            Log::error('Error al listar directorio distrital: ' . $e->getMessage());
            return $this->errorResponse('Error al obtener el directorio');
        }
    }
