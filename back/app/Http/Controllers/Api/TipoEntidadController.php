...
class TipoEntidadController extends Controller
{
    ...
    {
        try {
            $tipos = TipoEntidad::query()
                ->filter($request->only(['nombre']))
                ->orderBy('nivel_jerarquico')
                ->paginate($request->input('per_page', 15))
                ->withQueryString();

            return response()->json(['data' => $tipos], Response::HTTP_OK);
        } catch (\Exception $e) {
            Log::error('Error al listar tipos de entidad: ' . $e->getMessage());
            return $this->errorResponse('Error al obtener los tipos de entidad');
        }
    }
